...
export default function AdminDashboardPage() {
  ...
  const fetchDashboardData = async (isBackground = false) => {
    ...
    try {
      const [activitiesRes, quotesRes, messagesRes] = await Promise.all([
        fetch('/api/admin/activities?limit=15'),
        fetch('/api/quotes'),
        fetch('/api/contact'),
      ]);
      ...
      if (activitiesData.success) {
        ...
        if (fetchedActivities.length > 0) {
          ...
          if (initialLoadDoneRef.current && latestActivityIdRef.current && newest.id !== latestActivityIdRef.current) {
            // A genuine new activity arrived live!
  ...
  useEffect(() => {
    ...
    // Poll every 4.5 seconds when tab is active
  ...
  // Regular 10-second ticker to update relative timestamps smoothly
      ...
      >
        <div>
          ...
          <p className="text-xs text-black/60 mt-1">
            Real-time operations center for Top Grade Rice Millers. Activities update automatically without refreshing.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-sm border border-[#123D2A]/10 shadow-2xs overflow-hidden flex flex-col justify-between">
          <div>
            <div className="p-4 border-b border-black/5 flex items-center justify-between bg-[#FCFAF5]">
              <div className="flex items-center gap-2">
                ...
                {isLiveActive && (
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" title="Live stream is active" />
                )}
              </div>
            ...
            <div className="divide-y divide-black/5">
              {activities.length > 0 ? (
                ...
                <div className="p-12 text-center text-xs text-black/40 space-y-2">
                  ...
                  <p>{loading ? 'Connecting to live activity stream...' : 'No activities recorded yet. Submissions will appear here live.'}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-3 bg-[#FCFAF5] border-t border-black/5 flex items-center justify-between text-xs text-black/60">
            <span className="text-[11px]">Auto-monitored every 4.5 seconds</span>
